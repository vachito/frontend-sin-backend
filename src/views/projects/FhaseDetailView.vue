...
const btnText = ref('Agregar tarea')
const title = ref('Agregar Nueva Tarea')
...
<template>
  <Card class="max-w-100% bg-background">
    <CardHeader class="flex gap-3 items-center">
      <CardTitle class="">{{ phase.name }}</CardTitle>
    </CardHeader>
    ...
    <CardContent>
      ...
      <div class="flex justify-between my-5">
        <p>Responsable: {{ phase.responsible_id === 1 ? 'Juan' : '' }}</p>
        <p>Prioridad: {{ phase.weight }}</p>
